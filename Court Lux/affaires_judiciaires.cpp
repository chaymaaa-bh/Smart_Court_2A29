...
Affaires_judiciaires::Affaires_judiciaires(int id, QString Nom, QString Type, QString Horraire, QString Avancement, QString Faits, QString Decision, int Num)
{
    this->id = id;
    this->Nom = Nom;
    this->Type = Type;
...
//int Affaires_judiciaires::getNum(){return Num;}
...
//void Affaires_judiciaires::setNum(int Num){this->Num = Num;}
...
bool Affaires_judiciaires::ajouter(){
    ...
    QString id_string = QString::number(id);
        ...
        query.prepare("INSERT INTO AFFAIRE (id, nom, type, horraire, avancement, faits, decision, num) "
                      "VALUES (:id, :nom, :type, :horraire, :avancement, :faits, :decision, :num)");
...
bool Affaires_judiciaires::controle_d_existence(int id)
{
    QSqlQuery query;
    QString idd = QString::number(id);
    query.prepare("SELECT * FROM  affaire where id NOT IN (SELECT id FROM  affaire where id =: id)");
...
QSqlQueryModel* Affaires_judiciaires::Rechercher_aff(QString value)
{
     ...
     modal->setQuery("select * from AFFAIRE where id like '%"+value+"%' or nom like '%"+value+"%' or type like '%"+value+"%' or horraire like '%"+value+"%' or avancement like '%"+value+"%' or faits like '%"+value+"%' or decision like '%"+value+"%' or  or num like '%"+value+"%'");
     return modal;
}
...
QSqlQueryModel *Affaires_judiciaires::Trier_aff(QString croissance,QString critere)
{
    ...
    if(critere == "id" && croissance == "ASC")
        ...
        modal->setQuery("select * from AFFAIRE order by DECISION DESC ");
           ...
           modal->setQuery("select * from AFFAIRE order by NUM ASC ");
       else if(critere == "Num" && croissance == "DESC")
           modal->setQuery("select * from AFFAIRE order by NUM DESC ");
        ...
        modal->setQuery("select * from AFFAIRE order by decision");
           ...
           modal->setQuery("select * from AFFAIRE order by num");
       else if(critere == "Decision" && croissance == "")
           modal->setQuery("select * from AFFAIRE order by num");
