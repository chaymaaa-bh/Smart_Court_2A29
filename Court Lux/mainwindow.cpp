...
void MainWindow::on_login_clicked()
{
    ...
    if(qry.exec("select * from EMPLOYE where EMAIL ='"+ username +"' and MDP='"+password +"'"))
    {
    int count=0;
        if(qry.next())
        {
...
//Gestion employes
void MainWindow::effacer_champs2() //complete
{
   ui->le_id->clear();
   ui->le_nom->clear();
   ui->le_cin->clear();
   ui->le_prenom->clear();
   ui->le_naissance->clear();
   ui->le_nationalite->clear();
   ui->cb_role->setCurrentText("choisir");
   ui->cb_departement->setCurrentText("Choisir");
   ui->le_email->clear();
   ui->le_tel->clear();
   ui->le_mdp->clear();
...
void MainWindow::on_enregistrer_clicked() //modifier not complete
{
    ...
    QString Naissance = ui->le_naissance->text();
    QString Nationalite = ui->le_nationalite->text();
    QString tel = ui->le_tel->text();
    QString Email = ui->le_email->text();
    QString Departement = ui->cb_departement->currentText();
    QString role = ui->cb_role->currentText();
    QString mdp = ui->le_mdp->text();

if (ui->rb_ajouter->isChecked()) //if ajouter is clicked
   {
    Employe E(id, cin,  Nom, Prenom,Naissance , Nationalite, tel, Email, Departement, role, mdp);
    bool test= E.ajouter();
...
else  //if modifier is clicked

    {
        Employe E;
        bool test= E.modifier(id, cin, Nom, Prenom,Naissance , Nationalite, tel, Email, Departement, role, mdp);
          if(test)
          {
...
void MainWindow::on_pb_supprimer2_clicked() //completed
{
    Employe E;
...
void MainWindow::on_le_rechercher_textChanged() //completed
{
        QString value = ui->le_rechercher->text();
           Employe e;
           if(value!=NULL) ui->tab_emp->setModel(e.Rechercher_emp(value));
           else  ui->tab_emp->setModel(e.afficher());

}


void MainWindow::on_cb_tri_currentTextChanged() // completed
{
    Employe e;
    QString critere="", croissance="" ;


    if(ui->cb_tri->currentText()=="ID ASC")
       { critere="id"; croissance="ASC" ;}
    else  if(ui->cb_tri->currentText()=="ID DESC")
       { critere="id"; croissance="DESC" ;}
    else if(ui->cb_tri->currentText()=="Nom ASC")
    {critere="Nom"; croissance="ASC" ;}
    else if(ui->cb_tri->currentText()=="Nom DESC")
    {critere="Nom"; croissance="DESC" ;}

    if(ui->cb_tri->currentText() != "Tri par")
        ui->tab_emp->setModel(e.Trier_emp(croissance,critere));
...
void MainWindow::on_pb_pdf_clicked()
{
    ...
    QString value=ui->le_pdf->text();
         ...
         qry.exec();
         Mod->setQuery(qry);
         tab_emp.setModel(Mod);
         //c.closeconnection();

         QString strStream;
         QTextStream out(&strStream);

       /*  const int rowCount = tab_emp.model()->rowCount();
         const int columnCount =  tab_emp.model()->columnCount();*/
         ...
         out <<  "<html>\n"
                            ...
                            QString id = tab_emp.model()->data(tab_emp.model()->index(0, 0)).toString().simplified();
                            qDebug()<<id;
                            out << QString("<h3>ID : %1</h3>").arg((!id.isEmpty()) ? id : QString("&nbsp;"));

                            QString cin = tab_emp.model()->data(tab_emp.model()->index(0, 1)).toString().simplified();
                            out << QString("<h3>CIN : %1</h3>").arg((!cin.isEmpty()) ? cin : QString("&nbsp;"));

                            QString nom = tab_emp.model()->data(tab_emp.model()->index(0, 2)).toString().simplified();
                            out << QString("<h3>NOM : %1</h3>").arg((!nom.isEmpty()) ? nom : QString("&nbsp;"));

                            QString prenom = tab_emp.model()->data(tab_emp.model()->index(0, 3)).toString().simplified();
                            out << QString("<h3 bkcolor=0>PRENOM : %1</h3>").arg((!prenom.isEmpty()) ? prenom : QString("&nbsp;"));

                            QString naissance = tab_emp.model()->data(tab_emp.model()->index(0, 4)).toString().simplified();
                            out << QString("<h3 bkcolor=0>NAISSANCE : %1</h3>").arg((!naissance.isEmpty()) ? naissance : QString("&nbsp;"));

                            QString nationalite = tab_emp.model()->data(tab_emp.model()->index(0, 5)).toString().simplified();
                            out << QString("<h3 bkcolor=0>NATIONALITE : %1</h3>").arg((!nationalite.isEmpty()) ? nationalite : QString("&nbsp;"));

                            QString tel = tab_emp.model()->data(tab_emp.model()->index(0, 6)).toString().simplified();
                            out << QString("<h3 bkcolor=0>TELEPHONE : %1</h3>").arg((!tel.isEmpty()) ? tel : QString("&nbsp;"));


                            QString email = tab_emp.model()->data(tab_emp.model()->index(0, 7)).toString().simplified();
                            out << QString("<h3 bkcolor=0>EMAIL : %1</h3>").arg((!email.isEmpty()) ? email : QString("&nbsp;"));


                            QString depart = tab_emp.model()->data(tab_emp.model()->index(0, 8)).toString().simplified();
                            out << QString("<h3 bkcolor=0>DEPARTEMENT : %1</h3>").arg((!depart.isEmpty()) ? depart : QString("&nbsp;"));

                            QString role = tab_emp.model()->data(tab_emp.model()->index(0, 9)).toString().simplified();
                            out << QString("<h3 bkcolor=0>ROLE : %1</h3>").arg((!role.isEmpty()) ? role : QString("&nbsp;"));
...
void MainWindow::on_gestEmployes_clicked()
{
